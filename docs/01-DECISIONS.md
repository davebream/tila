...
**Amendment (v0.2 era):** A single-machine local SQLite backend (`@tila/backend-local`) is distinct from this prohibition. It targets co-located agents on one machine (laptop, VPS) using `bun:sqlite` with `BEGIN IMMEDIATE` transactions and WAL mode -- the same first-writer-wins correctness model as the DO, serialized via SQLite locking instead of DO single-threading. No multi-machine sync; no files in the project repo; no git coordination. The backend interfaces explicitly anticipate this (see Decision 1: "allow alternative implementations to be added in future versions"). `tila project create` remains the default for multi-machine teams. `tila project create --local` is the zero-setup path for solo agents on a single machine.
...
**v0.1 (kickoff):** tila ships DO-first architecture (entities + journal + claims in DO SQLite), D1 for auth tokens and idempotency, R2 with Worker-driven cleanup. Worker, read-only UI, `tila` CLI binary. Working schema-as-config. Smart Placement on. Cloudflare-only, no local mode.
