...
- **The action site validates the fence with strict equality.** A write is accepted only when its fence is *exactly* the current fence for that resource; any other value — lower (a stale lease that expired during work) **or** higher (a fence the resource never issued) — is rejected. The DO increments the fence on each acquire, so the caller must present the exact fence it received. Strict equality, not `>=`, is load-bearing for record-write safety: it rejects forged or out-of-order fences rather than waving through anything "recent enough".
