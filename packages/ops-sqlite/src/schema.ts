...
// (`@tila/backend-d1`); in embedded mode it folds into the same project SQLite
// file for same-transaction atomicity. This Drizzle model mirrors the DDL in
// `@tila/backend-embedded`'s MIGRATION_IDEMPOTENCY (version 1000) so the embedded
// backend can read/write idempotency rows via Drizzle instead of raw SQL.
