...
import { resolveEntityResource } from "./fence-ops";
...
export function release(
  ...
  db.transaction((tx) => {
    ...
    tx.delete(schema.claims).where(eq(schema.claims.resource, resource)).run();
...
export function countExpiredClaims(
  ...
  const rows = db.select().from(schema.claims).all();
  return rows.filter((row) => row.expires_at <= now).length;
}
