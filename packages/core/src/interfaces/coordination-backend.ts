 ...
 * is missing, expired, or held by a different principal/participant. Returning the full
...
export interface CoordinationBackend {
   ...
   * means the caller has LOST the claim (missing / expired / identity mismatch);
  ...
  heartbeat(info?: Record<string, unknown>): Promise<void>;
   ...
   * Use this instead of `listPresence` when you need a full participant inventory
