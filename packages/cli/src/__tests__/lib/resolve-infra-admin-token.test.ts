...
import { resolveInfraAdminToken } from "../../lib/resolve-destroy-plan";
...
describe("resolveInfraAdminToken", () => {
  it("prefers the environment variable over infra.toml", () => {
    const token = resolveInfraAdminToken(
      { ...baseInfra, infra_admin_token: "from-file" },
      "from-env",
    );
  ...
  it("returns null when neither env nor infra.toml provides a token", () => {
    expect(resolveInfraAdminToken(baseInfra, undefined)).toBeNull();
    expect(resolveInfraAdminToken(null, undefined)).toBeNull();
  });
});
